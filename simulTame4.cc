/*
 * NOTE: This is the enhanced/evolved engine from the first Gaia project adapted for the first Gaia2 movie
 */

/*
 * ffmpeg  -r 16 -f image2 -i  title-text/text_1_%05d.png -vcodec libx264 -crf 31 -pix_fmt yuv420p title-text.mp4 -y
 *
 * next should be 2x8 seconds + filler to 282 frames
 * ffmpeg  -r 16 -f image2 -i title-merged/final_%05d.png -vcodec libx264 -crf 31 -pix_fmt yuv420p title-merged.mp4 -y

 * ffmpeg  -r 16 -f image2 -i Gaia2-final2/Gaia2_%05d.png -vcodec libx264 -crf 31 -pix_fmt yuv420p title-merged.mp4 -y
 * ffmpeg  -r 16 -f image2 -i gaia2-final/y-%05d.png -vcodec libx264 -crf 31 -pix_fmt yuv420p video-final.mp4 -y
 * ffmpeg -f concat -i files2.txt -vcodec copy -acodec copy  gaia2-final.video.mp4 -y
 *
 * ffmpeg -i gaia2-final.video.mp4 -i Gaia2-final2/Gaia2.wav -vcodec copy -b:a 384000 Gaia2-1920x1080-ZZ-3x3.mp4 -y
 */
...
static inline void calcmean(int x, int y)
{
	...
	yx = (y-2)*MAXW+(x-2);
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=1; r += 1*grid[yx].r; g += 1*grid[yx].g; b += 1*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=4; r += 4*grid[yx].r; g += 4*grid[yx].g; b += 4*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=6; r += 6*grid[yx].r; g += 6*grid[yx].g; b += 6*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=4; r += 4*grid[yx].r; g += 4*grid[yx].g; b += 4*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=1; r += 1*grid[yx].r; g += 1*grid[yx].g; b += 1*grid[yx].b; }
	...
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+= 4; r +=  4*grid[yx].r; g +=  4*grid[yx].g; b +=  4*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=16; r += 16*grid[yx].r; g += 16*grid[yx].g; b += 16*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=24; r += 24*grid[yx].r; g += 24*grid[yx].g; b += 24*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=16; r += 16*grid[yx].r; g += 16*grid[yx].g; b += 16*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+= 4; r +=  4*grid[yx].r; g +=  4*grid[yx].g; b +=  4*grid[yx].b; }
	...
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=1; r += 1*grid[yx].r; g += 1*grid[yx].g; b += 1*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=2; r += 2*grid[yx].r; g += 2*grid[yx].g; b += 2*grid[yx].b; }
	yx++;
	if (yx>=0 && yx< MAXW*MAXH && xyok[yx]) { n+=1; r += 1*grid[yx].r; g += 1*grid[yx].g; b += 1*grid[yx].b; }
	...
	yx = y * MAXW + x;
	uint32_t rgb = mean[yx].r << 16 | mean[yx].g << 8 | mean[yx].b;
...
//					if (r != 255 || g != 0 || b != 255)
	...
	if (arg_image) {
		...
		for (uint32_t ix=0; ix<numEval; ix++) {
			...
			if (mmapfd < 0) {
				// nearest RGB
				(*nearestRGB)[r << 16 | g << 8 | b][0] = r << 16 | g << 8 | b;
				(*nearestDist)[r << 16 | g << 8 | b][0] = 0;
			}
			...
			if (rmap[r << 16 | g << 8 | b] != -1)
				fprintf(stderr,"dupcol: %02x%02x%02x\n", r,g,b);
			rmap[r << 16 | g << 8 | b] = yx;
		}
	} else {
	...
	*/
	...
	double leader_frames = 20;
	double log_fps_start = log(0.1);
	double log_fps_end = log(16.0);
	...
	double fps;
	{
		double pos = totalEval / (numEval * leader_frames);
	...
	for (;;) {
		...
		for (int w=0; w<numEval; w++) {
			...
			int found = -1;
			uint32_t rgb = grid[worse].r << 16 | grid[worse].g << 8 | grid[worse].b;
			for (int s = 0; s < NUMSLOT; s++) {
			...
			todoOut = 0;
			todoRGB[todoIn] = rgb;
			todoVersion[rgb] = gVersion;
			todoIn++;
			uint32_t bestRGB = 0xffffffff;
			uint32_t bestDist = 0xffffffff;
			uint32_t rgbworse = rgb;
			while (todoOut < todoIn) {
				uint32_t rgb =  todoRGB[todoOut++];

				if (root[rgb] != -1) {
					int dist = distance2(rgbworse, rgb);
					if (dist < bestDist) {
						bestRGB = rgb;
						bestDist = dist;
					}
				...
				if (bestRGB == 0xffffffff || bestRGB == rgb) {
					for (int k = 0; k < 5; k++) {
						uint32_t z = (*nearestRGB)[rgb][k];
						assert(z <= 0xffffff);
						if (todoVersion[z] != gVersion) {
							todoRGB[todoIn] = z;
							todoVersion[z] = gVersion;
							todoIn++;
						}
					}
				}
			...
			printf("%06x %06x %d %d\n", rgbworse, bestRGB, bestDist, todoIn);
#endif
		}
		...
		for (int ii=0; ii<numEval; ii++) {
			int i = evalseq[ii];
			for (int j = root[mean[i].r<<16 | mean[i].g<<8 | mean[i].b]; j != -1; j = mean[j].next) {
				if (i==j)
					cnt++;
			}
			assert(cnt>0);
